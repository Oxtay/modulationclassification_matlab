...
iteration = 1000;    % Number of runs per simulation
...
c         = rayleighchan(1/10000,100); % Defining the Rayleigh channel
...
% for alp=0.01:0.01:signif
    for j=1:iteration
  % Defining the waveform and how the signal will be tested in a
  % channel.
...
%         X = awgn(X_1,db);
          X = filter(c,X_1);      % Pass signal through channel 
        [H,Threshold,t] = normality_scenario3(real(X), signif);
        for count = 1:n
            if H(count)==0
