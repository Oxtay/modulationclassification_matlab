...
clear all;
phase_rotate = zeros(30,1);
Sum_of_all = 0;
runs = 500;
for trial = 1:runs
    for Offset = 1:30

D_parameter = [0,0,0,0,Offset/100,0];
...
% axis fill
indice = max(C1)-min(C1);
phase_rotate(Offset) = atan(indice(1)/indice(2))*180/pi;
    end;
        Sum_of_all = phase_rotate+Sum_of_all;
end;
%% Plotting
Offset = 0.01:0.01:0.3;
phase_calculated = Sum_of_all/(runs);
plot(Offset', phase_calculated, '-bo','MarkerSize',5)
grid on
axis('square')
axis([0 .3 45 45.18]);
% title('Calculating I-Q phase imbalance using the proposed algorithm in 16-QAM')
xlabel('Actual phase imbalance (%)');
ylabel('Calculated angle due to offset (deg)')