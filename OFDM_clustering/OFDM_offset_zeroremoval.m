function OFDM_offset_zeroremoval(D_parameter)
...
% =====Removing zeros from the data to reduce the error in finding
% cluster centers =================
% i=1;
% while i <= size(farray)
%    if abs(farray(i)) < 2*sqrt(0.25)
%        farray(i) = [];
%    end
%    i = i+1;
% end
...
farray_nonzero = farray(abs(farray)> .25);
